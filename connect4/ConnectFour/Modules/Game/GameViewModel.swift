...
final class GameViewModel: ObservableObject {
    ...
    init(coordinator: GameCoordinator, container: GameContainer, configuration: GameConfiguration) {
        ...
        let youPlayer = Player(
            ...
            color: playerWearsYellow ? .warning : .red,
            managedByAI: false
        )
        
        let otherPlayer = Player(
            name: Self.otherPlayerName(gameMode: gameMode),
            color: playerWearsYellow ? .red : .warning,
...
extension GameViewModel {
    ...
    func restart() {
        ...
        let youPlayer = Player(
            name: Self.youPlayerName,
            color: self.youPlayer.color == .red ? .warning : .red,
            managedByAI: false
        )
        
        let otherPlayer = Player(
            name: Self.otherPlayerName(gameMode: gameMode),
            color: otherPlayer.color == .warning ? .red : .warning,
